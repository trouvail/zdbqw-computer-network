...
void send_package(SOCKET& socketClient, SOCKADDR_IN& servAddr, int& servAddrlen, char* message, int len, int &order) // 发送单个数据报，order为序列号
{
    HEADER header;
...
{
    int packagenum = len / MAXSIZE + (len % MAXSIZE != 0); // 计算一共要发送数据报的次数
