...
int main()
{
    ...
    char* fileNum = new char[1];
    RecvMessage(server, server_addr, len, fileNum);
    int w = fileNum[0] - '0'; // 接收文件个数，转换char为int
    cout << "本次共需接受" << w << "个文件，开始接收" << endl;
    char** name = new char*[w]; // 存储文件的名称以及内容，在接收完全部数据后再写文件
    char** data = new char*[w];
    int* namelen = new int[w];
    int* datalen = new int[w];
    for (int x = 0; x < w; x++)
    {
        name[x] = new char[20];
        data[x] = new char[100000000];
        namelen[x] = RecvMessage(server, server_addr, len, name[x]);
        datalen[x] = RecvMessage(server, server_addr, len, data[x]);
    }
    ...
    for (int x = 0; x < w; x++)
    {
        ...
        a += 'v';
        a += '_';
        for (int i = 0; i < namelen[x]; i++) // 将接收到的文件的名字信息赋值给a
        {
            a = a + name[x][i];
        }
        ofstream fout(a.c_str(), ofstream::binary); // 用二进制方式打开文件
        for (int i = 0; i < datalen[x]; i++)
        {
            fout << data[x][i];
        }
        fout.close();
